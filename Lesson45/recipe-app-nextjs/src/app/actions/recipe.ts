...
const recipeSchema = z.object({
  ...
  strInstructions: z.string().min(50, 'Cooking instructions should be at least 50 chars long.'),
  strMealThumb: z.string().url('Please provide a correct url pointing to an images')
})

export async function saveRecipe(prevState: SaveRecipeActionResponse | null, formData: FormData): Promise<SaveRecipeActionResponse> {
  ...
  try {
    ...
    console.log('New recipe saved:', validatedData.data);
    ...
    console.error('An unexpected error occured when trying to same a recipe', error);
    return {
      success: false,
